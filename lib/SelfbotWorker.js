...
let odaily = "owo daily", opray = "owo pray <@889402266912686090>", oinv = "owo inv", olb = "owo lb all", ouse = "owo use", ordinary = ["whunt", "w battle", "w hunt", "wbattle", "owo hunt" , "owo battle"], oother = ["owo run", "owo pup", "owo piku"], ohb = "owo huntbot"
...
async function aGem(param1, param2, param3) {
    ...
    collector.on("collect", async (m) => {
        inv = m.content.split("`")
        gem1 = inv.filter(elm => elm.match(/^05[1-5]$/))
        gem2 = inv.filter(elm => elm.match(/^06[5-9]$/))
        gem3 = inv.filter(elm => elm.match(/^07[2-6]$/))
        box = inv.indexOf("050") >= 0 ? true : false
        if(!global.config.autoLootbox) box = false
        if(box) {
            global.channel.sendTyping();
            await sleep(randomInt(680, 3400));
